...
```{r}
{plot(smooth.spline(p_logistic, Wmod, df = 4))
abline(0, 1)}
```
...
Good propensity estimates should have calibration
that are close to the diagonal.
...
of fast-enough convergeing regression adjustments).
...
Here, we use the amlinear package for cross-fitting the lasso. This
package is still under development (not yet on CRAN), so you'll
need to install it from github. Note that, when installing from
github, you need to also install dependencies by hand,
e.g., to get CVXR, just run install.packages("CVXR").

---


```{r load_amlinear}
#install_github("swager/amlinear") # install amlinear package
library(amlinear)
```
...
glmnet.fit.propensity = amlinear:::crossfit.cv.glmnet(Xmod.int, Wmod,
                                                      family = "binomial")
theta.hat = amlinear:::crossfit.predict(glmnet.fit.propensity)
p_lasso = 1/(1 + exp(-theta.hat))

{plot(smooth.spline(p_lasso, Wmod, df = 4))
abline(0, 1)}
```
...
Finally, let's review all our estimators. Recall that
tauhat_rct is the gold standard from the RCT.
