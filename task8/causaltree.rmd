---
title: "Heterogeneity in voter preferences for female candidates"
author: "Marek Chadim"
date: "10-06-2024"
output:
...
```{r}
load("politician_gender_prefs.rdata")
data <- df
...
```
## Run regressions of picked_cand_a on cand_a_female and one of each of the other included regressions, main effects and interactions. 
...
### Why can’t we use the standard errors from these estimates for analyzing hetergeneity?

We need to correct for the fact that we are testing for multiple hypotheses, or we will end up with many false positives. We can use Bonferroni or Romano-Wolf correction to adjust the p-values for multiple testing.
...
No, the interaction is not signiﬁcant.
